...
#Business Problem ----------------------------------------------------------------------------------
# The ACME corporation is considering moving into agricultural inputs in Ghana. 
# Whom to target their sales efforts, hired you to analyze and determines agricultural profit there.
# Especially interested in the effect of household educational attainment and the characteristics of
# the local area has on profit, but if you find other results of interest, add it.
# Left choice of explanatory variables and variable definitions to you, but to be able to compare 
# across regions they want at least one specification that examines profit per area unit
# (for example, acre, hectare, or square meters). 

#Analysis-------------------------------------------------------------------------------------------
# Use Ghana Living Standard Survey 4 from 1998/99, including documentation that comes with survey.
# Raw stata data from Ghana Statistical Service, expect a fair number of data issues. 
# Not all data are relevant. Read documentation before looking data. 
# Some of data are calculated aggregates from the World Bank.  

#Memorandum:---------------------------------------------------------------------------------------
# Write a (500 words) summary of findings in memorandum format based on analysis. Not interested in
# statistical analysis; want results and interpretation of finding to decide on their plans. 
# Must interpret and write up results that makes sense to them; they have no statistical training.
# The memorandum should NOT contain technical jargon. Include clearly labeled and explained 
# table(s), graph(s), or both to highlight relevant points about the data and statistical analysis.

#Statistical Analysis Paper:------------------------------------------------------------------------
# The statistical analysis for internal discussions of plans.  Imperative that statistical analysis
# clearly and thoroughly documented. Must describe each step and why. Methods, hypotheses, and 
# results of the analyses you do should be clearly explained and labeled in the appendix.

#R code:--------------------------------------------------------------------------------------------
# Submit R project with all files needed to reproduce your analysis (except for the original data).
# Can use GitHub, zip project and upload it. Essential that project run on another computer 
# Document code thoroughly. Do not change the names of the raw data files. 

#Grading--------------------------------------------------------------------------------------------
# Grade depends on ability to:
...
land_chars_df <- land_chars_df %>% 
  mutate( hh_land_owned_acres = case_when(
    s8aq3 == 3 ~ s8aq4 / 9,
    s8aq3 == 2 ~ s8aq4,
...
View(ag_profit_land_educ_df)
summary(ag_profit_land_educ_df)
...
summary(profit_grouped_by_clust)
